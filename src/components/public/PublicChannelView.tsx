...
function aggregate(snaps: ChannelDailySnapshot[]) {
  ...
  const additiveKeys = [
    ...
    "orders", "grossRevenue", "netRevenue", "totalDiscounts",
  ...
  if (totals.orders > 0 && totals.revenue > 0) totals.avgOrderValue = totals.revenue / totals.orders;
...
export default function PublicChannelView({
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <KPICard label="CPA" value={fCur(curr.cpa, currency)} delta={calcDelta(curr.cpa || 0, prev.cpa || 0)} deltaInverse />
  ...
  if (channel === "ECOMMERCE") {
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <KPICard label="Revenue" value={fCur(curr.revenue || curr.grossRevenue, currency)} delta={calcDelta(curr.revenue || curr.grossRevenue || 0, prev.revenue || prev.grossRevenue || 0)} />
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <KPICard label="Open Rate" value={fPct(curr.openRate)} delta={calcDelta(curr.openRate || 0, prev.openRate || 0)} />
          <KPICard label="Click Rate" value={fPct(curr.clickRate)} delta={calcDelta(curr.clickRate || 0, prev.clickRate || 0)} />
...
function renderTopCampaigns(snapshots: ChannelDailySnapshot[]) {
  ...
  return (
    <div className="bg-special/50 border border-white/5 rounded-lg overflow-hidden">
        ...
        <tbody>
          {campaigns.map((c, i) => (
            <tr key={i} className="border-t border-white/5">
              ...
              <td className="px-4 py-2 text-right text-white/60">${c.spend.toFixed(0)}</td>
              <td className="px-4 py-2 text-right text-white/60">{c.conversions}</td>
              <td className="px-4 py-2 text-right text-white/60">${c.revenue.toFixed(0)}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function renderTopProducts(snapshots: ChannelDailySnapshot[]) {
        ...
        <tbody>
          {products.map((p, i) => (
            <tr key={i} className="border-t border-white/5">
              ...
              <td className="px-4 py-2 text-right text-white/60">${p.revenue.toFixed(0)}</td>
            </tr>
          ))}
        </tbody>
      </table>
