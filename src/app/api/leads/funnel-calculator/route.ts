...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        // Fetch leads for this client in date range (with index fallback)
        let leadsSnap;
        try {
            leadsSnap = await db.collection("leads")
        ...
        let leads = leadsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() }) as Lead);
        // Client-side date filter (in case index fallback was used)
        leads = leads.filter(l => (l.createdAt || "") >= startStr);
        ...
        let metaDocs: FirebaseFirestore.QueryDocumentSnapshot[] = [];
        try {
            ...
            // No META data or missing index — skip cost metrics
        }

        const totalMetaSpend = metaDocs.reduce((sum, doc) => {
        ...
        const totalImpressions = metaDocs.reduce((sum, doc) => sum + (doc.data().metrics?.impressions || 0), 0);
        const totalClicks = metaDocs.reduce((sum, doc) => sum + (doc.data().metrics?.clicks || 0), 0);
        ...
        return NextResponse.json({
            ...
            qualificationRate: Math.round(qualificationRate * 1000) / 10,
        ...
        console.error("[Funnel Calculator] Error:", error?.message || String(error));
        return NextResponse.json({ error: "Failed to compute funnel data" }, { status: 500 });
    }
}
